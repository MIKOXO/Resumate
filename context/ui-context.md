...
- **Dashboard**: two-panel layout, not a nav sidebar. This app has one real screen — a sidebar implies navigation between sections that don't exist here.
  ...
  - **Left panel**: fixed width (~300px), full height below top bar, right border separator. Scrollable flat list of the user's prospects — no grouping or nesting. "Add prospect" action pinned at the top. Each prospect row shows the name with a selection highlight and a 3-dot menu for "Replace resume" / "Delete".
...
- **Auth screens** (Signup, Login, Verify Email, Forgot Password, Reset Password): centered card on `--bg-base`, max-width ~400px, `--bg-surface` background, `12px` radius. Verify Email screen uses a 6-digit code input (segmented, one box per digit) with a "resend code" text link below. The resend link is disabled and shows a live countdown ("Resend in 47s") for 60 seconds after each send, matching the backend's cooldown — becomes clickable again once it hits zero.
